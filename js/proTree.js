    ...
    Tree.prototype.init = function() {
        ...
        this.bindEvent();
			...
			Tree.prototype.proHTML = function(arr) {
				...
				arr.map(function(item) {
					...
					if(item.child && item.child.length > 0) {
						lihtml += "<i ischek='true' class='" + self.mouIconClose + "' ></i>" +
			...
			Tree.prototype.bindEvent = function() {
                $(".menuUl li i").closest("li").children(".menuUl").hide();
                $(".menuUl li i").attr("ischek", 'false');
				var self = this;
                //$(".menuUl > li i ").closest("i").removeClass(self.mouIconOpen).addClass(self.mouIconClose);
